...
- 安装依赖：通过，astrbot-proactive-core==0.1.0 可 import；远端访问 PyPI 出现 SSL EOF，实机本次使用本地 wheel fallback 安装 core。发布后已在本机临时干净 venv 从 PyPI 安装 astrbot-plugin-proactive-chat==0.1.6，确认自动安装 core==0.1.0，且 wheel 包含 main.py、metadata.yaml、_conf_schema.json、requirements.txt。
...
- 远端机器访问 PyPI 有 SSL EOF，因此远端 AstrBot 仍是本地 wheel fallback 安装 core；本机干净 venv 已完成 PyPI 安装验证。
